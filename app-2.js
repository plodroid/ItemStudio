function generateGive(){
  ...
  if(p.startsWith('components')){
    ...
    if(state.enchants.length){
      const levels=state.enchants.map(x=>`${JSON.stringify(ns(x.id))}:${Math.max(0,Math.floor(safeNum(x.level,1)))}`).join(',');
  ...
  if(state.enchants.length){
    if(p==='nbt_modern') tag.push(`Enchantments:[${state.enchants.map(x=>`{id:${JSON.stringify(ns(x.id))},lvl:${Math.floor(safeNum(x.level,1))}s}`).join(',')}]`);
    else tag.push(`ench:[${state.enchants.map(x=>`{id:${LEGACY_ENCHANT_IDS[stripNs(x.id)] ?? 0}s,lvl:${Math.floor(safeNum(x.level,1))}s}`).join(',')}]`);
  }
  if(state.attributes.length){
  ...
  if(hideEnchants) tag.push(`HideFlags:${p==='nbt_legacy'?1:1}`);
  ...
  return `/give @p ${p==='nbt_legacy'?oldItemId(item):ns(item)}${tag.length?`{${tag.join(',')}}`:''}${data}`;
}

function effectSlotTarget(slot){
  if(profile().startsWith('components')){
...
function generateEffects(){
  ...
  return state.effects.map((e,i)=>{
    ...
    if(p.startsWith('components')){
      return `/execute as @a if items entity @s ${slot} ${ns(state.item)} run effect give @s ${effect} 2 ${amp} true`;
    }
    if(cmp(state.version,1,13,0)>=0){
      ...
      return `/execute as @a if entity @s[nbt=${pred}] run effect give @s ${effect} 2 ${amp} true`;
    }
